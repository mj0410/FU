...
param_grid = dict(units=units, neurons=neurons, activation=activation, filters=filters)
...
cnn_lstm = create_model(units=params['units'], neurons=params['neurons'], activation=params['activation'], filters=params['filters'])
history = cnn_lstm.fit(x_train, y_train, epochs = 10, validation_split = 0.2, verbose=2)

score = cnn_lstm.evaluate(x_test, y_t, verbose=0)

"""precision-recall curve"""

probs = cnn_lstm.predict(x_test)[:,1]
...
result_df.to_csv ('CNN-LSTM_result.csv', index = False, header=True)
