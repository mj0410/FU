...
def create_model(filters=10, neurons=10, activation='relu'):
    # create model
    model = Sequential()
...
filters = [64, 128, 256]
neurons = [32, 50, 64, 128, 256]
activation = ['relu', 'tanh']

param_grid = dict(neurons=neurons, filters=filters, activation=activation)
...
cnn = create_model(filters=params['filters'], neurons=params['neurons'], activation=params['activation'])
history = cnn.fit(x_train, y_train, epochs = 10, validation_split = 0.2, verbose=2)

score = cnn.evaluate(x_test, y_t, verbose=0)

"""precision-recall curve"""

probs = cnn.predict(x_test)[:,1]
