...
def str_preprocess(x):
  if len(eval(x))==1:
    x = x.replace("\n", "")
    x = x.replace(" ", ", ")
  return x

def str_preprocess_pred(x):
  x = x.replace("\n", "")
  x = x.replace(" ", ", ")
  return x
...
def int_or_none(v):
    # convert int to boolean type for argparser input
    if v is None:
       return None
    if v.lower() == 'none':
        return None
...
def series_reindex_todict(list1, list2, series):
    arrays = [list1, list2]
...
def load_tmp_df(load_path, tmp_path, name, table=False):
    ...
    if os.path.exists(pkl_file):
        ...
        df = pd.read_csv(join(load_path, "{}.csv".format(name)), keep_default_na=False)
...
def load_tmp_parquet(load_path, tmp_path, name, table=False):
    start = time.time()
    pkl_file = join(tmp_path, "{}.pkl".format(name))
    if os.path.exists(pkl_file):
        print("{} pickle file found, loading...".format(pkl_file))
        ...
        #process and save pkl
        print("{} pickle file not found, creating...".format(pkl_file))
        df = pd.read_parquet(join(load_path, "{}.parquet".format(name)))
