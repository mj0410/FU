...
def extract_sherlock_features(df_dic):
    ...
    for field_order, field_name in enumerate(df.columns):
        ...
        all_field_features[field_order]['row_index'] = row_index
        all_field_features[field_order]['field_id'] = '{}:{}'.format(dataset_id,
                                                                     field_id)  # field id in the filterd table
        all_field_features[field_order]['header'] = field_name
        all_field_features[field_order]['header_c'] = utils.reduce_header2(field_name)
        n_values = len(v)
...
def extract_sherlock_features_pred(df_dic):
